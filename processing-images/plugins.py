...
def mono(image, sepia=False):
    ...
    height = len(image)
    width = len(image[0])
    assert sepia == False or sepia == True
    if sepia == False:
        for row in range(height):
            for col in range(width):
                pixel = image[row][col]
                brightness= 0.3 * pixel.red + 0.6 * pixel.green + 0.1 * pixel.blue
                image[row][col].red = int(brightness)
                image[row][col].green = int(brightness)
                image[row][col].blue = int( brightness)
    else:
        for row in range(height):
            for col in range(width):
                pixel = image[row][col]
...
def flip(image,vertical=False):
    ...
    if vertical == True:
        for row in range(height//2):
            for col in range(width):
                y=copy.deepcopy(image[row][col])
                image[row][col] = image[height-1-row][col]
                image[height-1-row][col] = y
    else:
        for row in range(height):
            for col in range(width//2):
                y=copy.deepcopy(image[row][col])
                image[row][col] = image[row][width-1-col]
                image[row][width-1-col] = y
...
def transpose(image):
    ...
    for col in range(width):
        r=[]
        for row in range(height):
            temp=copy.deepcopy(image[row][col])
            r.append(temp)
        image1.append(r)
    image.clear()
    height1=len(image1)
    ...
    for row in range(height1):
        ...
        for col in range(width1):
            temp = copy.deepcopy(image1[row][col])
            r.append(temp)
        image.append(r) 
    return True
...
def vignette(image):
    ...
    crow=height//2
    ccol=width//2
    h= math.sqrt((0 - crow) * (0 - crow) + (0 - ccol) * (0 - ccol))
    h1=abs(h)
    for row in range(height):
        for col in range(width):
            dist = math.sqrt((row - crow) * (row - crow) + (col - ccol) * (col - ccol))
            d=abs(dist)
            factor= 1 - math.pow((d / h),2)
            image[row][col].red=int(image[row][col].red * factor)
            image[row][col].green=int(image[row][col].green * factor)
            image[row][col].blue=int(image[row][col].blue * factor)
    return True
...
def blur(image,radius=5):
    ...
    for row in range(height1):
        for col in range(width1):
            ...
            while rowb < radius:
                while colb < radius:
                    if row + rowb < height and col + colb < width:
                        
                        averageRed = averageRed + image[row + rowb][col + colb].red
                        averageRed = averageRed + image[row - rowb][col - colb].red
                    
                        averageGreen = averageGreen + image[row + rowb][col + colb].green
                        averageGreen = averageGreen + image[row - rowb][col - colb].green
                                        
                        averageBlue = averageBlue + image[row + rowb][col + colb].blue
                        averageBlue = averageBlue + image[row - rowb][col - colb].blue
                                        
                        averageAlpha = averageAlpha + image[row + rowb][col + colb].alpha
                        averageAlpha = averageAlpha + image[row - rowb][col - colb].alpha
                    
                        colb = colb + 1
                rowb = rowb + 1
        image1[row][col].red = averageRed
        image1[row][col].green = averageGreen
        image1[row][col].blue = averageBlue
        image1[row][col].alpha = averageAlpha

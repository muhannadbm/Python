...
def bad_visibility(visibility,minimum):
    ...
    try:
        if visibility == 'unavailable':
            return True
        vismin = visibility['minimum']
        if visibility['units'] == 'FT':
            vismin = visibility['minimum'] / 5280
        if vismin < minimum:
            return True
        else:
            return False
    except:
        vismin = visibility['prevailing']
        if visibility['units'] == 'FT':
            vismin = visibility['prevailing'] / 5280
        if vismin < minimum:
            return True
        else:
            return False
...
def bad_winds(winds,maxwind,maxcross):
    ...
    if winds == 'unavailable':
        return True
    try:
        speedgust = winds['gusts']
    except:
        speedgust = 0
    if winds == 'calm':
        return False
    speedwind = winds['speed']
    speedcross = winds['crosswind']
    
    if speedwind == 'calm':
        return False

    if winds['units'] == 'MPS':
        speedwind = speedwind * 1.94384
        speedcross = speedcross * 1.94384
        speedgust = speedgust * 1.94384
    worse = max(speedwind,speedgust)
    if worse > maxwind or speedcross > maxcross:
        return True
    else:
        return False
...
def bad_ceiling(ceiling,minimum):
    ...
    for i in range(skylen):
        if ceiling[i]['type'] == 'broken' or ceiling[i]['type'] == 'overcast' or ceiling[i]['type'] == 'indefinite ceiling':
            if ceiling[i]['height'] < minimum:
                return True
            else:
                return False
    return False
...
def get_weather_violation(weather,minimums):
    ...
    if weather == None:
        return 'Unknown'
    visibilityprob = bad_visibility(weather['visibility'],minimums[1])
    windpro = bad_winds(weather['wind'],minimums[2],minimums[3])
    skypro = bad_ceiling(weather['sky'],minimums[0])
    if weather == None:
        return 'Unknown'
    if visibilityprob == False and windpro == False and skypro == False:
        return ''
    if visibilityprob == True and windpro == False and skypro == False:
        return 'Visibility'
    elif visibilityprob == False and windpro == True and skypro == False:
        return 'Winds'
    elif visibilityprob == False and windpro == False and skypro == True:
        return 'Ceiling'
    else:
        return 'Weather'
...
def list_weather_violations(directory):
    ...
    for r in range(1,row):
        takeoff = lessons[r][3]
        takeofftime = utils.str_to_time(takeoff)
        pilot = lessons[r][0]
        pilotlist = utils.get_for_id(pilot,students)
        instructed = lessons[r][2]
        if len(instructed)!=0:
            instructed = True
        area = lessons[r][6]
        daytime = utils.daytime(takeofftime,daycycle)
        pilotcred = pilots.get_certification(takeofftime,pilotlist)
        flightmin = pilots.get_minimums(pilotcred, area, instructed, lessons[r][5] == 'VFR', daytime,minimums)
        weathercondition = get_weather_report(takeofftime,weather)
        viol = get_weather_violation(weathercondition,flightmin)
        templist = lessons[r]
        if viol != '':
            templist.append(str(viol))
